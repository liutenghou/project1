...
% If the goal statement starts with "is it...", change the order of
% the first two words to "it is...", then let the established sentence
% DCG transform the statement into an attribute.
goal(Goal,[is,it|A],D):- sentence(Attr,[it,is|A],E),
	build_goal(Attr,Goal), D=E.

% If the goal statement starts with "does it...", change the first two
% words into just "it...", then let the established sentence DCG
% transform the statement into an attribute.
goal(Goal,[does,it|A], D):- sentence(Attr,[it|A],E),
	build_goal(Attr,Goal), D=E.

% If the goal statement starts with "what...", pass it to the question
% phrase DCG defined below.
goal(Goal) --> [what], qphrase(Goal).

% For the question "What is it?", build attr(is_a,X,[]).
qphrase(Goal) --> [is],[it],
    {build_variable_goal(Goal,is_a)}.

% For the question "What does it have?", build attr(has_a,X,[]).
qphrase(Goal) --> [does],[it],[have],
    {build_variable_goal(Goal,has_a)}.

% Helper functions to build the attr()s for goals.
